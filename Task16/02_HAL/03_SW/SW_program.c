...
void SW_voidInit(SW_Type SW_Config)
{
    ...
    if (SW_Config.Pull_State == SW_INPUT_PULL_UP)
    {
...
u8 SW_u8GetPressed(SW_Type SW_Config)
{
    ...
    if ((SW_Config.Pull_State == SW_INPUT_PULL_UP) || (SW_Config.Pull_State == SW_PULL_UP))
    {
    ...
    else if (SW_Config.Pull_State == SW_PULL_DOWN)
    {
        DIO_enumGetPinValue(SW_Config.Port, SW_Config.Pin, &LOC_u8PinVal);
